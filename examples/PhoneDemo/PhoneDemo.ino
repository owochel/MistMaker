...
// board's room: (1) over-the-air with a {t:"room"} command authorized by the
// printed words; (2) hardware fallback — hold the button at power-on to re-run
// the WiFi setup page.
//
...
// (The Extension Kit has no button — the pull-down keeps the pin quiet.)
//
...
// panel.
//
// Libraries (Library Manager): MistMaker >= 2.2.0,
...
void handleTest() {
  applyLevel(portal.arg("on") == "1" ? BUTTON_LEVEL : 0);   // local hardware check, no cloud
...
void handleSave() {
  ...
  prefs.putString("name", portal.arg("name"));   // friendly label (blank = the private name)
...
//   {"t":"room","id":"<id>","key":"<words>","room":"<new>"}  OTA room move
void onCommand(const char* msg, size_t len) {
  ...
  if (!strcmp(t, "set")) {
    ...
    if (forMe && doc["level"].is<int>()) {
      lastCmd = millis();
      const int lvl = constrain(doc["level"].as<int>(), 0, 100);
    ...
    if (mine.is<int>()) {
      lastCmd = millis();
      const int lvl = constrain(mine.as<int>(), 0, 100);
    ...
    const char* ckey  = doc["key"];
    const char* croom = doc["room"];
    if (!cid || strcmp(cid, deviceId)) return;         // not addressed to us
    if (ckey && croom && !strcmp(ckey, privateRoom) && croom[0]
        && strlen(croom) <= 32 && strcmp(croom, room.c_str())) {
...
void onWsEvent(WStype_t type, uint8_t* payload, size_t len) {
  switch (type) {
    ...
    case WStype_TEXT:         onCommand((const char*)payload, len); break;
    default: break;
  }
}
